...
- `push()`: `动态存储（storage）数组`拥有`push()`成员，可以在数组最后添加一个`0`元素，并返回该元素的引用。
- `push(x)`: `动态存储（storage）数组`拥有`push(x)`成员，可以在数组最后添加一个`x`元素。
- `pop()`: `动态存储（storage）数组`拥有`pop()`成员，可以移除数组最后一个元素。
